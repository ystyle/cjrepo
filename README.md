...
一个兼容 `cjpm` 的仓颉包私有仓库服务，支持包的发布、下载和索引查询。

## 特性

- 📦 **包管理** - 完整的包发布、下载、索引查询功能
- 🏢 **组织管理** - 支持多组织隔离，控制用户上传权限
- 🌐 **上游源代理** - 支持配置官方中央库，自动代理和缓存包，可清理缓存释放空间
...
#### 组织管理
![组织管理](assets/admin_org.png)

创建/编辑/删除组织，添加/移除成员，设置默认组织。支持多组织隔离和细粒度权限控制。
...
配置官方中央库，支持自动代理和缓存。当本地没有包时会自动从上游拉取，可查看缓存统计并清理过期缓存释放空间。
...
**使用场景**：
- 私有仓库：配置官方中央库，本地私有包和公共包统一管理
- 级联部署：支持多级 cjrepo 部署，下级可配置上级 cjrepo 为上游
